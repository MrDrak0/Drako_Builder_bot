...
const permissionNamesByBit = new Map(Object.entries(PermissionFlagsBits).map(([name, bit]) => [String(bit), name]));
function permissionKey(permission) {
    const bit = new PermissionsBitField(permission).bitfield.toString();
    return permissionNamesByBit.get(bit);
}
...
export function toOverwritePayload(overwrite) {
    const payload = {};
