...
import { CATEGORY_SPECS, CHANNEL_AFK } from '../config/channelSpec.js';
...
function overwriteData(overwrites, roleIdByName, everyoneId, label) {
...
const setupFullCommand = {
    ...
    async execute({ interaction }) {
        ...
        const currentChannels = [...(await guild.channels.fetch()).values()].filter((channel) => channel !== null);
        ...
        const existingCategories = currentChannels.filter((channel) => channel.type === ChannelType.GuildCategory && specCategoryNames.has(normalizeName(channel.name)));
        const existingChannels = currentChannels.filter((channel) => channel.type !== ChannelType.GuildCategory && specChannelNames.has(normalizeName(channel.name)));
        ...
        for (const entry of ROLE_SPECS) {
            try {
                const role = await rateLimited(() => guild.roles.create({
                    name: entry.name,
                    color: entry.color,
                    hoist: entry.hoist,
        ...
        for (const categorySpec of CATEGORY_SPECS) {
            try {
                const category = (await rateLimited(() => guild.channels.create({
                ...
                for (const channelSpec of categorySpec.channels) {
                    try {
                        ...
                        await rateLimited(() => guild.channels.create({
        ...
        const afkChannel = [...guild.channels.cache.values()].find((channel) => (channel.type === ChannelType.GuildVoice || channel.type === ChannelType.GuildStageVoice) && normalizeName(channel.name) === normalizeName(CHANNEL_AFK));
        if (afkChannel) {
            try {
