...
/** One permission overwrite entry: target a role by name ('@everyone' for the everyone role). */
...
interface ChannelSpec {
  ...
  /** Defaults to 'text'. */
  kind?: 'text' | 'voice';
  /** Per-channel overwrites (explicit, per the spec tables). */
  overwrites?: ChannelOverwrite[];
}

interface CategorySpec {
  name: string;
  /** Category-level baseline overwrites (cascade to every child). */
  overwrites?: ChannelOverwrite[];
  channels: ChannelSpec[];
}

/** Shorthand builders so the tables below read cleanly. */
const allow = (roleName: string, ...permissions: PermissionResolvable[]): ChannelOverwrite => ({ roleName, allow: permissions });
const deny = (roleName: string, ...permissions: PermissionResolvable[]): ChannelOverwrite => ({ roleName, deny: permissions });
