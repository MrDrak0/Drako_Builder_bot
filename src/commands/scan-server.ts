import { ChannelType, Colors, EmbedBuilder, Guild, NonThreadGuildBasedChannel, PermissionFlagsBits, Role, SlashCommandBuilder } from 'discord.js';
import { Command } from '../handlers/commandLoader.js';
import { normalizeName, specNameSets } from '../utils/setupUtils.js';

const FIELD_VALUE_LIMIT = 1024;
const FIELD_NOTE = (n: number): string => `\n…and ${n} more — full list in the file`;
 ...
 */
function fitField(lines: string[]): { value: string; truncated: boolean } {
  if (lines.length === 0) return { value: '(none)', truncated: false };
  let value = '';
  for (let i = 0; i < lines.length; i++) {
    ...
    if (candidate.length + noteLen > FIELD_VALUE_LIMIT) break;
    value = candidate;
    if (remaining === 0) return { value, truncated: false };
  }
 ...
 */
const scanServerCommand: Command = {
  ...
  async execute({ interaction }) {
    ...
    const rawChannels = [...(await guild.channels.fetch()).values()].filter((channel): channel is NonThreadGuildBasedChannel => channel !== null);
...
function overwriteInfo(channel: NonThreadGuildBasedChannel, roleById: Map<string, Role>): OverwriteInfo[] {
 ...
 */
function collectIssues(channels: ChannelInfo[], roleById: Map<string, Role>, guild: Guild): string[] {
  const issues: string[] = [];
  ...
  for (const name of specCategoryNames) {
    if (!categoryNames.has(name)) issues.push(`Missing category: ${name}`);
  }
  for (const name of specChannelNames) {
    if (!presentChannelNames.has(name)) issues.push(`Missing channel: ${name}`);
  }
...
function channelsWithOverwrites(channels: ChannelInfo[]): string[] {
  const lines: string[] = [];
  for (const channel of channels) {
...
function dumpPlain(channels: ChannelInfo[], categories: ChannelInfo[], roles: Role[], issues: string[]): string {
  const out: string[] = [];
  for (const category of categories) {
    ...
    for (const child of channels.filter((c) => c.parentId === category.id)) {
      ...
      for (const overwrite of child.overwrites) {
        const flags = [];
        if (overwrite.allow.length) flags.push(`allow(${overwrite.allow.join(', ')})`);
        if (overwrite.deny.length) flags.push(`deny(${overwrite.deny.join(', ')})`);
  ...
  if (uncategorized.length) {
    ...
    for (const channel of uncategorized) out.push(`  ${channel.name}  [${channel.id}]`);
  }
  out.push('== Roles ==');
